...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'ammarkhan8217@gmail.com' ")
...
def TrackImages():
    ...
    face_timer = {}  # dictionary to store face IDs and their appearance times

    while timer < 10:  # 5 minutes
        ret, im = cam.read()
    ...
    # filter out faces that appeared for less than 10 seconds
    filtered_face_timer = {k: v for k, v in face_timer.items() if v > 5}

    # write attendance records for filtered faces
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    ...
    if exists:
        with open("Attendance\Attendance_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            for serial in filtered_face_timer:
                ID = df.loc[df['SERIAL NO.'] == serial]['ID'].values
                ID = str(ID)
                ID = ID[1:-1]
                bb = df.loc[df['SERIAL NO.'] == serial]['NAME'].values
                bb = str(bb)
                bb = bb[2:-2]
                attendance = [str(ID), '', bb, '', str(date), '', str(timeStamp)]
...
message3 = tk.Label(window, text="Face Recognition Based Attendance Monitoring System" ,fg="white",bg="red" ,width=55 ,height=1,font=('comic', 29, ' bold '))
message3.place(x=10, y=10)

datef = tk.Label(window, text=day + "-" + mont[month] + "-" + year, fg="white", bg="blue",
                 width=20, font=('comic', 15, ' bold '))
datef.pack(fill='both', expand=True)
datef.place(relx=0.30, rely=0.09)

clock = tk.Label(window, fg="white", bg="blue", width=20, font=('comic', 15, ' bold '))
...
def send_email():
    ...
    from_email = "ammarkhan8217@gmail.com"  # Update with your email
    password = "cquv kxwt eshx hudm"  # Update with your email password
